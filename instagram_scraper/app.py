...
class TagDataframe(object):
    ...
    def appendDF(self, hashtag, filename, check_display_url, shortcode):
...
class InstagramScraper(object):
            ...
            try:
                while True:
                    ...
                    if end_cursor:
                        ...
                        print('end cursor is entered!!! : ', end_cursor)
                        return
            except ValueError:
    ...
    def augment_node(self, node):
        ...
        if node['is_video'] and 'video_url' in node:
            ...
            node['urls'] = self.__get_urls_from_details(details, node)
       ...
       # print("len urls : ", len(node['urls']))
        ...
        #if len(node['urls']) < 1:
            #print("")
            #print(" no urls. so I made it. Lets check")
            #node['urls'] = self.__get_urls_from_details_se(details, node)
            #print("urls : ", node['urls'])
        if len(node['urls']) < 1:
            print("only display_url -  shortcode : ", node['shortcode'])
            node['urls'] = [self.get_original_image(node['display_url'])]
            #print(node)
            #print("urls : ", node['urls'])
        ...
        #    #details = self.__get_media_details(node['shortcode'])
        #    #print(details)
        #    print("shortcode : ", node['shortcode'])
        #print("urls : ", node['urls'])
        #rint("display_url : ", node['display_url'])
    ...
    def __get_urls_from_details(self, details, node):
    ...
    def __get_media_details(self, shortcode):
        ...
        #print("====__get_media_details start====")
        #print(json.loads(resp)['graphql']['shortcode_media'])
        #print("====__get_media_details end====")
