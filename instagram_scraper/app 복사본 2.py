...
class TagDataframe(object):
    ...
    def __init__(self, csv):
        ...
        try:
            ...
            print("There is no csv file. make new DataFrame.")
            self.df_flag = False
    ...
    def checkDF(self, hashtag, filename):
        ...
        df_check = bool(self.df.query(query_expr)['filename'].any())
        #temp_df_check = bool(self.temp_df.query(query_expr)['filename'].any())
        
        return df_check 

    def getLastTag(self, hashtag):
        if self.df_flag:
            query_expr = "(hashtag == @hashtag)"
            df = self.df.query(query_expr)
            df_tail = df.tail(1)['end_cursor']
            if df_tail.any():
                return df_tail.values[0]
            else:
                return ''
        else:
            return ''

    def appendDF(self, hashtag, filename):
        data = {'hashtag' : hashtag, 'filename' : filename, 'end_cursor' : self.end_cursor}
        self.temp_df = self.temp_df.append(data, ignore_index=True)

    def addHashtag(self, hashtag):
        self.hashtags.add(hashtag)

    def getHashtagName(self):
        return '_'.join(self.hashtags)

    def setEndCursor(self, end_cursor):
    ...
    def updateMainDF(self):
        df = self.df.append(self.temp_df, ignore_index=True)
        return df

    def saveTempDFtoCsv(self):
        save_dir = "./temp_tagdata.csv"
        self.temp_df.to_csv(save_dir, index=False)
        print("new data saved to : ", save_dir)

#    def saveDFtoCsv(self):
#        df = self.df.append(self.temp_df, ignore_index=True)
#        df.to_csv('./' + self.getHashtagName + ".csv", index=False)

    def saveToCsv(self, filename):
        df = self.df.append(self.temp_df, ignore_index=True)
        #df.to_csv('./' + filename + '.csv', index=False)
        #df.to_csv('./tagdata.csv', index=False)
    ...
    def checkItemisinDF(self, hashtag, item):
        ...
        for url in item['urls']:
            ...
            fullfilename = str(filename + extension)
            if self.df_flag:
                check = bool(check & self.checkDF(hashtag, fullfilename))
                if not(self.checkDF(hashtag, fullfilename)):
                    self.appendDF(hashtag, fullfilename)
            else:
                self.appendDF(hashtag, fullfilename)
            return check
...
class InstagramScraper(object):
                ...
                for item in tqdm.tqdm(media_generator(value), desc='Searching {0} for posts'.format(value), unit=" media",
                    ...
                    if self.filter_locations:
                        if item.get("location") is None or self.get_key_from_value(self.filter_locations, item["location"].get("id")) is None:
                            continue
                    #if bool(re.search(r"_n.mp4", ''.join(item['urls']))):
                    #    print("=====================================")
                    #    print("item : ", item)
                    #    print("item['is_video'] : ", item['is_video'])
                    #    print("item['urls'] : ", item['urls'])

                    #print("total : ", ((item['is_video'] is False and 'image' in self.media_types) or (item['is_video'] is True and 'video' in self.media_types)) and self.is_new_media(item) and not(bool(re.search(r"_n.mp4", ''.join(item['urls'])))) and not(td.checkItemisinDF(value, item)))
                    #print("(item['is_video'] is False and 'image' in self.media_types) or (item['is_video'] is True and 'video' in self.media_types) : ", (item['is_video'] is False and 'image' in self.media_types) or (item['is_video'] is True and 'video' in self.media_types))
                    #print("not(td.checkItemisinDF(value, item) : ", not(td.checkItemisinDF(value, item)))

                    if ((item['is_video'] is False and 'image' in self.media_types) or \
                                (item['is_video'] is True and 'video' in self.media_types)
                        ) and self.is_new_media(item) and \
                        not(bool(re.search(r"_n.mp4", ''.join(item['urls'])))) and not(self.td.checkItemisinDF(value, item)):
    ...
    def query_hashtag_gen(self, hashtag):
        end_cursor = self.td.getLastTag(hashtag)
        if end_cursor != '' :
            print("scraping start with :", end_cursor)
        return self.__query_gen(QUERY_HASHTAG, QUERY_HASHTAG_VARS, 'hashtag', hashtag, end_cursor)
    ...
    def __query(self, url, variables, entity_name, query, end_cursor):
        ...
        if resp is not None:
            ...
            if payload:
                ...
                self.td.setEndCursor(end_cursor)
    ...
    def augment_node(self, node):
        ...
        #tony edited
        #elif not('__typename' in node) and ('display_url' in node):
        #    print("typename is disapeared! ")
        #    node['urls'] = [self.get_original_image(node['display_url'])]
        else:
            if details is None:
                details = self.__get_media_details(node['shortcode'])
            ...
            if details:
                ...
                if '__typename' in details and details['__typename'] == 'GraphSidecar':
                    urls = []
        ...
        #print("")
        #print("graph video  : ", '__typename' in node and node['__typename'] != 'GraphImage')
       # print("len urls : ", len(node['urls']))
        if len(node['urls']) > 1:
            #print(node)
            print("")
            print(node['urls'])
            quit()
        #print("__typename in node : ", '__typename' in node)
        #print("node['__typename'] == GraphImage : ", node['__typename'] )
        #print("self.get_original_image(node['display_url']) : ", self.get_original_image(node['display_url']))
    ...
    def get_logger(level=logging.DEBUG, dest='', verbose=0):
        ...
        print("logger is setted :", dest + 'instagram-scraper.log')
